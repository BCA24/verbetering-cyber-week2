...
app = Flask(__name__)
with open("secretkey.txt", "r") as f: # OWASP 2
    secret_key = f.read()
app.secret_key = secret_key

app.config['SESSION_COOKIE_SECURE'] = True # OWASP 5
app.config['SESSION_TYPE'] = 'filesystem'  # Use filesystem session type
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///databases/event_calendar.db'
...
def userLogin():
    if request.method == "POST":
        ...
        if user:
            ...
            print('Failed login attempts:', session['failed_login_attempts'])
            # Check if the failed login attempts have reached the threshold
            if session['failed_login_attempts'] >= 3:
                # Trigger an alert (this could be an email, SMS, log event, etc.)
...
def createAgenda():
    if 'user' not in session or session['user_role'] != 'admin': # OWASP 1
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            if session['failed_login_attempts'] >= 3:
                ...
                print("ALERT: Multiple failed login attempts detected!")
            flash("Invalid username or password")
    # Add a return statement to handle the case where the request method is not "POST"
    return render_template("adminLogin.html", failed_login_attempts=session.get('failed_login_attempts', 0))

@app.route("/test")
def test():
    # Your test route code here
    pass
